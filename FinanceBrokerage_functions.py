...
def FinanceBrokerage_lister(url):
    ...
    artlist = soup.find('div', class_='listing listing-blog listing-blog-1 clearfix columns-1')  # Finds list of article URLs
...
def FinanceBrokerage_writer():
    BaseUrl = "https://www.financebrokerage.com/category/stocks/"  # base url from which to find articles
    URLlist = FinanceBrokerage_lister(BaseUrl) # Function created to reduce code, makes URL list
    ...
    for url in URLlist:  # Iterates over all URLs in list for writing to .txt
        ...
        if  (os.path.isfile(f'.\\TextFiles\\{title}.txt')):
            ...
            article = MotleyPage.find('div', class_='bialty-container')  # Find particular section of webpage
            #full_content = article.find('section',{'id': "full_content"})  # Creates object for pulling content from section
